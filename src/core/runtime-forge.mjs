...
import { forgeDynamicRuntimeSupport } from './runtime-dynamic-routes.mjs';
...
export function forgeRuntimeProject(projectDir,plan){const runtime=plan.selection.runtime.id,root=path.join(projectDir,'runtime');ensure(root);let files;if(runtime==='astro')files=astro(root,plan);else if(runtime==='next')files=next(root,plan);else if(runtime==='vite-react')files=vite(root,plan);else throw new Error(`Unsupported runtime: ${runtime}`);files.push(...copyVisualAssets(projectDir,root));const dynamicSupport=forgeDynamicRuntimeSupport(root,plan,runtime);files.push(...dynamicSupport.files);const packageResolution=writePackageResolution(root,runtime);files.push('package-resolution.json');const manifest={schema:'webforge.runtime-forge.v5',runtime,generated:true,visualParity:true,multiPageBlueprint:true,dynamicDataRoutes:dynamicSupport.status==='PASS',dynamicSupport,dependencyPolicy:'trusted exact-version baseline; install/build separately attested',packageResolution,files};fs.writeFileSync(path.join(root,'runtime.manifest.json'),JSON.stringify(manifest,null,2)+'\n');return {...manifest,files:[...files,'runtime.manifest.json']};}
